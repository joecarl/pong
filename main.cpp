...
void IniciarPunto(int plyrNum, int first, element *ball, playerP *players[], int sc)
{
    ...
    players[0]->medlen=MEDLEN; players[1]->medlen=MEDLEN;
    ...
    do{
        ...
        if(events.type == ALLEGRO_EVENT_TIMER)
        {
            ...
            if(gScreen==OVER){
                ...
                if(keys[ALLEGRO_KEY_Y]){gScreen=GAME;load_screen[GAME]=true;}
                if(keys[ALLEGRO_KEY_N])gScreen=MENU;
            }
            ...
            if(gScreen==MENU){
                ...
                if(keys[ALLEGRO_KEY_4]){players=2;isClient=true;load_screen[CONN]=true;gScreen = CONN;}
                if(keys[ALLEGRO_KEY_C]&&0){
                  //CAMBIAR RESOLUCION, bad performance
                    ...
                    font=al_load_ttf_font("../Resources/arial.ttf",scale*9 ,0) ;
                    buffer = al_create_bitmap(scale*resX, scale*resY);
                    gScreen=MENU;
                  
                }
                if(keys[ALLEGRO_KEY_ESCAPE]){gScreen=0; finish=true;}
                else if(keys[ALLEGRO_KEY_1] || keys[ALLEGRO_KEY_2] || keys[ALLEGRO_KEY_3]) {gScreen=GAME;load_screen[GAME]=true;paused=false;}
